...
export const fetchParties = (id = null, stime, etime, minamnt, maxamnt, limit) => {
  return new Promise(resolve => {
    return pool.connect((err, client, done) => {
      if (err) return resolve(null);
      client.query(`SELECT parties.*, users.fname, users.lname FROM parties WHERE ${id ? `id IN (SELECT party_id FROM party_guests WHERE guest_id = ${id}) OR` : ''} type = 0 LEFT JOIN users ON users.id = parties.id ${ limit ? `LIMIT ${limit}` : '' }`, async (err, result) => {
        if (err) return resolve(null);
